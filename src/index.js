...
const handleDigitsOnly = (value) => String(value || '')?.replace(/\D+/g, '');

const handleLuhnCheck = (pan) => {
  const value = handleDigitsOnly(pan);
...
const handleNormalizeExpiry = (expirationDate) => {
...
const handleNormalizeHolderName = (name) => {
...
export const scanPaymentCard = async ({ permission, scannerText, cardFrameColor } = {}) => {
  ...
  if (!handleLuhnCheck(cardNumberDigits)) {
    const err = new Error('Invalid card number detected. Please try again.');
    err.code = 'E_INVALID_CARD_NUMBER';
    throw err;
  };
  const expiry = handleNormalizeExpiry(response?.expirationDate);
  const holderName = handleNormalizeHolderName(response?.cardHolderName);
